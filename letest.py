...
def handle_attack(message):
    ...
    if message.chat.id != int(GROUP_ID):
        bot.reply_to(message, f"🚫 JOIN OUR GROUP TO USE THE BOT❌\n🔗 𝐉𝐨𝐢𝐧 𝐍𝐨𝐖: https://t.me/vicky08oct10 {CHANNEL_USERNAME}")
        return

    if not is_user_in_channel(user_id):
        bot.reply_to(message, f"❗ **𝐉𝐎𝐈𝐍 𝐊𝐑𝐎** {CHANNEL_USERNAME} 🔥")
        return

    if pending_feedback.get(user_id, False):
        bot.reply_to(message, "😡 **𝐒𝐂𝐑𝐄𝐄𝐍𝐒𝐇𝐎𝐓 SEND !** 🔥\n🚀 *AFTER ATTACK SEND PIC HERE TO USE ANOTHER ATTACK!*")
        return

    # Check if an attack is already running
    if is_attack_running(user_id):
        bot.reply_to(message, "⚠️ **WAIT BRO ATTACK RUNNING!** ⚡")
        return
    ...
    try:
        ...
        if user_status not in ['member', 'administrator', 'creator']:
            bot.reply_to(message, f"❌ **𝐘𝐨𝐮 𝐌𝐔𝐒𝐓 𝐉𝐎𝐈𝐍 𝐎𝐔𝐑 𝐂𝐇𝐀𝐍𝐍𝐄𝐋 𝐅𝐈𝐑𝐒𝐓!**\n"
                                  f"🔗 **𝐉𝐨𝐢𝐧 𝐇𝐞𝐫𝐞:** [https://t.me/vicky08oct10] {CHANNEL_USERNAME})")
            return  
...
👉 [𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝙂𝙧𝙤𝙪𝙥](https://t.me/vicky08oct10) 🚀🔥"""
